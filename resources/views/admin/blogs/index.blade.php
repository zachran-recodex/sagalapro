...
@section('content')
    ...
    <div class="flex items-center justify-between flex-wrap gap-2 mb-6">
        <h4 class="text-default-900 text-lg font-medium">Blogs</h4>

        <div class="md:flex bg-info px-6 py-2 rounded-md hidden items-center gap-3 text-sm font-semibold">
            <a href="{{ route('admin.blogs.create') }}" class="text-sm text-white font-medium text-default-700">Add new
                Blog</a>
        </div>
    </div>
    <!-- Page Title End -->
                        ...
                        <tbody>
                            @forelse ($blogs as $blog)
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-3">{{ $blog->description }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-3 text-center text-gray-500">No blogs Available
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
