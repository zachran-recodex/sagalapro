<x-layout.admin>
    <div class="flex flex-wrap items-center justify-between gap-2 mb-6">
        ...
        <ul class="flex items-center gap-[6px]">
            ...
            <li class="font-medium dark:text-white">Hero Section - Edit</li>
        </ul>
    </div>
    ...
    <div class="col-span-12">
        <div class="card border-0 overflow-hidden">
            <div class="card-header border-b border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-700 py-4 px-6 flex items-center flex-wrap gap-3 justify-between">
                <h5 class="card-title text-lg mb-0">Edit Hero Section</h5>
                <a href="{{ route('admin.hero-sections.index') }}" class="btn btn-primary text-sm btn-sm px-3 py-3 rounded-lg flex items-center gap-2">
                    <iconify-icon icon="ion:arrow-back-outline" class="icon text-xl line-height-1"></iconify-icon>
                    BACK
                </a>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.hero-sections.update', $heroSection) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-4">
                    @csrf
                    @method('PUT')

                    <div class="col-span-6">
                        <label for="title" class="form-label">Title</label>
                        <input type="text" id="title" name="title" value="{{ old('title', $heroSection->title) }}" class="form-control border border-neutral-200 dark:border-neutral-600 w-full rounded-lg" required>
                    ...
                    <div class="col-span-6">
                        ...
                        <img src="{{ Storage::url($heroSection->image) }}" alt="{{ $heroSection->title }}" class="border rounded-lg w-full h-32 object-cover">
                    </div>
                    ...
                    <div class="col-span-6 flex items-center">
                        <label class="inline-flex items-center cursor-pointer">
                            ...
                            <!-- Checkbox for status -->
                            <input type="checkbox" id="status" name="status" value="1" class="sr-only peer" {{ old('status', $heroSection->status) ? 'checked' : '' }}>
